...
class Paket extends CI_Controller
{
  ...
  public function getAjax($id_bab)
  {
    ...
    $mapel = $this->mapel->get($data['bab']->mapel_id)->row();
    $data = [];
    $no = @$_POST['start'];
    foreach ($list as $paket) {
      ...
      $row[] = $paket->created;
      $row[] = $paket->updated;
      $row[] = '<a  href="' . site_url("soal/index/$paket->id_paket/$mapel->id_mapel") . '" class="btn btn-primary has-ripple"><i class="feather mr-2 icon-edit"></i>Daftar Soal<span class="ripple ripple-animate" style="height: 112.65px; width: 112.65px; animation-duration: 0.7s; animation-timing-function: linear; background: rgb(255, 255, 255) none repeat scroll 0% 0%; opacity: 0.4; top: -38.825px; left: -2.85833px;"></span></a>
