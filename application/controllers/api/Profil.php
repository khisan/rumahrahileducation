...
class Profil extends BD_Controller
{
  ...
  public function index_put()
  {
    ...
    // );
    // $post['id_siswa'] = $this->put('id_siswa');
    $id_siswa = $this->put('id_si$id_siswa');
    $jenjang_id = $this->put('jenjang_id');
    $kelas_id = $this->put('kelas_id');
    $post['nama'] = $this->put('nama');
    $post['sekolah'] = $this->put('sekolah');
    $post['image'] = $this->put('foto');
    ...
    if (!empty($_FILES['foto']['name'])) {
      ...
      $post['image'] = null;
      $hasil = $this->siswa->updateProfilRest($post, $id_siswa);
      var_dump($hasil);
      return $this->set_response([
        'status' => REST_Controller::HTTP_OK,
        'message' => "Data Berhasil Diubah",
